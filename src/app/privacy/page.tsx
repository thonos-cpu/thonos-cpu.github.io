...
export default function PrivacyPage() {
  return (
    <main className="shell privacy-page" id="main">
      ...
      <p className="privacy-lead">This site uses a small first-party analytics system to understand traffic and improve the portfolio. Data is stored in Athanasios’s Supabase project and is not sold or used for advertising.</p>
      <PrivacyControls />
      <section><h2>Anonymous visit history</h2><p>The site records page views, which long-page sections enter the viewport, approximate foreground time, device class, coarse country code when supplied by the host, and live activity within a two-minute window. Network address and user-agent are transformed into a one-way keyed hash; the raw values are not stored. Vercel Speed Insights may collect aggregate performance measurements while analytics are enabled.</p></section>
